...
minimize  totCost/10+ totDistance*10 + 100000*totVeh; //new objective value by Myoungju Park
subject to {
//OptVehCnt==totVeh;
 totVeh<=5;
...
forall (i in aJobs: i.t=="c") 
	sum(j in aJobs, p in pair: i!=j && j.sid!="D0" &&
		i.id==p.id1 && j.id==p.id2) X[p] == 1; // (2)
//forall (i in aJobs: i.t=="f") sum(j in aJobs: i!=j && j.sid!="D0") X[i,j] <= 1; // (3)
forall (i in aJobs: i.t=="f")
	sum(j in aJobs, p in pair: i!=j && j.sid!="D0" &&
		i.id==p.id1 && j.id==p.id2) X[p] <= 1; // (3)

//forall (j in aJobs: j.t=="c" || j.t=="f")  	// (4)
// 	sum(i in aJobs: i!=j && i.sid!="D0") X[j,i] == sum(i in aJobs: i!=j && i.sid!="DN") X[i,j];
forall (j in aJobs: j.t=="c" || j.t=="f")  	// (4)
 	sum(i in aJobs, p in pair: i!=j && i.sid!="D0" && i.id==p.id2 && j.id==p.id1) X[p] == 
	sum(i in aJobs, p in pair: i!=j && i.sid!="DN" && i.id==p.id1 && j.id==p.id2) X[p];
...
forall (i,j in aJobs, p in pair: i!=j && (i.t=="c" || i.sid=="D0") && j.sid!="D0" && i.id==p.id1 && j.id==p.id2) // (5)
	A[i]+(t[i,j]+i.s)*X[p] - l0*(1-X[p]) <= A[j]; 
...
//New constraints by Myoungju Park
forall (i in aJobs, t in Times: i.t=="f") {
	L[i][t] == sum(r in 0..t)(S[i][r] - F[i][r]);
}

//New constraints for triples by Myoungju Park
forall (tr in triple) {
	sum(p in pair: (p.sid1==tr.sid1 && p.sid2==tr.sid2) || (p.sid1==tr.sid2 && p.sid2==tr.sid3)) X[p] <= 1;
}
...
writeln("i" +"\t" + "j"+"\t"+  "d" +"\t"+ "est" + "\t"+ "lst" + "\t" + "A" +"\t" + "D" +"\t" + "CG" +"\t" + "U" +"\t" + "Y" +"\t" + "q" );
for (var p in pair)
	if ( X[p]==1) for (var j in aJobs) if(p.id2==j.id) 
      	writeln( p.sid1  +"\t"+  p.sid2  + "\t" + p.d 
                  + "\t" +p.e2 + "\t" + p.l2  + "\t" + A[j]  + "\t" + D[j]  + "\t" + CG[j] + "\t" + U[j]  + "\t" + Y[j] + "\t" +j.q) ;
}